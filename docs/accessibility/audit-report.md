...
### 2.11 Color Contrast (WCAG 1.4.3 — Contrast Minimum) — UX-14.2.7
...
### 2.12 Color Contrast — Системные исправления (UX-14.2.9)

**Нарушение:** Системные проблемы контраста в компонентах:

| Компонент | Комбинация | Ratio | Результат | Исправление |
|-----------|-----------|-------|-----------|-------------|
| StatsCard | `dark:text-slate-400` на `dark:bg-slate-800` | ~5.3:1 | ⚠️ Гранично для small text | `dark:text-slate-300` |
| Input (disabled) | `text-slate-500` на `bg-slate-50` | ~3.7:1 | ❌ FAIL | `disabled:text-slate-600` |
| Table (sort icon) | `text-slate-400` на `bg-slate-50` | ~2.5:1 | ❌ FAIL | `text-slate-500` |
| Table (ellipsis) | `text-slate-400` на `bg-white` | ~2.6:1 | ❌ FAIL | `text-slate-500` |
| DataGrid (sort icon) | `text-slate-400` на `bg-slate-50` | ~2.5:1 | ❌ FAIL | `text-slate-500` |
| DataGrid (search icon) | `text-slate-400` на `bg-white` | ~2.6:1 | ❌ FAIL | `text-slate-500` |
| DataGrid (checkbox) | `text-slate-400` на `bg-white` | ~2.6:1 | ❌ FAIL | `text-slate-500` |
| DataGrid (page btn) | `dark:text-slate-400` на `dark:bg-slate-800` | ~5.3:1 | ⚠️ Гранично | `dark:text-slate-300` |
| Sidebar (subtitle) | `text-slate-400` на `bg-slate-900` | ~6.9:1 | ✅ PASS | Усилено до `text-slate-300` |
| Sidebar (close) | `text-slate-400` на `bg-slate-900` | ~6.9:1 | ✅ PASS | Усилено до `text-slate-300` |

**Исправление:** Систематически заменены все `text-slate-400` на `text-slate-500`
на светлом фоне и на `text-slate-300` в тёмной теме. Создана утилита
[`contrast.ts`](../frontend/src/lib/contrast.ts) для программной проверки контраста.

**Файлы:**
- [`frontend/src/lib/contrast.ts`](../frontend/src/lib/contrast.ts) — новая утилита
- [`frontend/src/components/ui/StatsCard.tsx`](../frontend/src/components/ui/StatsCard.tsx:38-99) — dark:text-slate-400 → dark:text-slate-300
- [`frontend/src/components/ui/Input.tsx`](../frontend/src/components/ui/Input.tsx:42) — disabled:text-slate-500 → disabled:text-slate-600
- [`frontend/src/components/ui/Table.tsx`](../frontend/src/components/ui/Table.tsx:49-243) — text-slate-400 → text-slate-500
- [`frontend/src/components/ui/DataGrid.tsx`](../frontend/src/components/ui/DataGrid.tsx:275-719) — text-slate-400 → text-slate-500/300
- [`frontend/src/components/layout/Sidebar.tsx`](../frontend/src/components/layout/Sidebar.tsx:99) — text-slate-400 → text-slate-300
...
| 1.4.3 Contrast Minimum | ✅ | Все цвета ≥ 4.5:1 (UX-14.2.9) |
...
- Интегрировать `contrast.ts` в CI/CD pipeline для автоматической проверки.
...
| [`frontend/src/lib/contrast.ts`](../frontend/src/lib/contrast.ts) | Утилита проверки контраста WCAG 2.1 |
...
| Файл | Изменения |
|------|-----------|
| [`frontend/src/lib/contrast.ts`](../frontend/src/lib/contrast.ts) | Новая утилита: `relativeLuminance`, `contrastRatio`, `isAA`, `isAAA` |
| [`frontend/src/components/ui/StatsCard.tsx`](../frontend/src/components/ui/StatsCard.tsx) | `dark:text-slate-400` → `dark:text-slate-300` (subtitle, trend, MiniStatsCard) |
| [`frontend/src/components/ui/Input.tsx`](../frontend/src/components/ui/Input.tsx) | `disabled:text-slate-500` → `disabled:text-slate-600` (Input, Select, Textarea) |
| [`frontend/src/components/ui/Table.tsx`](../frontend/src/components/ui/Table.tsx) | `text-slate-400` → `text-slate-500` (sort icon, empty state, pagination, ellipsis) |
| [`frontend/src/components/ui/DataGrid.tsx`](../frontend/src/components/ui/DataGrid.tsx) | `text-slate-400` → `text-slate-500`/`300` (sort icon, search, checkbox, pagination, empty state) |
| [`frontend/src/components/layout/Sidebar.tsx`](../frontend/src/components/layout/Sidebar.tsx) | `text-slate-400` → `text-slate-300` (subtitle, close button) |

---

*Audit проведён в рамках тикетов UX-14.2.7 и UX-14.2.9.*
