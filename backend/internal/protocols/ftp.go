...
// safePath очищает путь и проверяет, что он не выходит за пределы rootPath.
// Возвращает абсолютный безопасный путь или os.ErrPermission при попытке path traversal.
func (d *FTPDriver) safePath(userPath string) (string, error) {
	// "/" — валидный корень FTP, остальные абсолютные пути — path traversal
	if strings.HasPrefix(userPath, "/") && userPath != "/" {
		if d.logger != nil {
			d.logger.Warn("FTP path traversal attempt: absolute path", "path", userPath)
		}
		return "", os.ErrPermission
	}

	cleaned := filepath.Clean(filepath.Join(d.rootPath, userPath))
	rootClean := filepath.Clean(d.rootPath)

	if !strings.HasPrefix(cleaned, rootClean) {
		if d.logger != nil {
			d.logger.Warn("FTP path traversal attempt", "path", userPath, "resolved", cleaned)
		}
		return "", os.ErrPermission
	}
...
func (d *FTPDriver) PutFile(ctx *ftp.Context, destPath string, data io.Reader, offset int64) (int64, error) {
	fullPath, err := d.safePath(destPath)
	if err != nil {
		return 0, err
	}
