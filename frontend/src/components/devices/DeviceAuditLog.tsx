...
import {
  Clock, Download, Filter, X, Loader2, AlertTriangle, Wrench,
} from '../ui/Icons';

// ═══════════════════════════════════════════════════════════════════════
// Chips for event type filter
// ═══════════════════════════════════════════════════════════════════════

const EVENT_TYPE_CHIPS = [
  { value: '', label: 'Все', color: 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400' },
  ...
  { value: 'MAINTENANCE',   label: 'Обслуживание',  color: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' },
  { value: 'ASSIGN',        label: 'Назначение',    color: 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-400' },
  { value: 'FIRMWARE',      label: 'Прошивка',      color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' },
  { value: 'SYSTEM',        label: 'Система',       color: 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-400' },
  { value: 'CREATE',        label: 'Регистрация',   color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' },
];
...
function actionToLabel(action: string): string {
  const map: Record<string, string> = {
    CREATE: 'Устройство зарегистрировано',
    UPDATE: 'Параметры изменены',
    DELETE: 'Устройство удалено',
    STATUS_CHANGE: 'Статус изменён',
    ASSIGN: 'Назначение изменено',
...
export const DeviceAuditLog: React.FC<DeviceAuditLogProps> = ({ deviceId }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Не удалось загрузить историю устройства');
      setEntries([]);
    } finally {
      setLoading(false);
    }
  ...
  const exportCSV = () => {
    ...
    link.href = url;
    link.download = `device-audit-${deviceId}-${new Date().toISOString().slice(0, 10)}.csv`;
    link.click();
    ...
    <Card>
      <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium">История устройства</span>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {filter ? 'Нет событий по выбранному фильтру' : 'История устройства пуста'}
            </p>
          </div>
        ) : (
