...
import { Download, FileText, Calendar, Clock, FileSpreadsheet, Loader2 } from '../ui/Icons';
                                        ...
                                        onClick={async () => {
                                            ...
                                            try {
                                                if (report.fileUrl) {
                                                    ...
                                                    // P1-PERF-BUNDLE.3: Dynamic import ExcelJS — MIT, ~350KB
                                                    const ExcelJS = await import('exceljs');
                                                    const workbook = new ExcelJS.Workbook();
                                                    const worksheet = workbook.addWorksheet('Report Meta');
                                                    worksheet.columns = [
                                                        { header: 'Report ID', key: 'Report ID', width: 20 },
                                                        { header: 'Report Name', key: 'Report Name', width: 30 },
                                                        { header: 'Type', key: 'Type', width: 20 },
                                                        { header: 'Date Range', key: 'Date Range', width: 25 },
                                                        { header: 'Generated At', key: 'Generated At', width: 25 },
                                                        { header: 'Generated By', key: 'Generated By', width: 20 },
                                                        { header: 'Original Size', key: 'Original Size', width: 18 },
                                                    ];
                                                    worksheet.addRows([{ 'Report ID': report.id, 'Report Name': report.name, 'Type': report.type, 'Date Range': report.dateRange, 'Generated At': report.generatedAt, 'Generated By': report.generatedBy, 'Original Size': report.size }]);
                                                    const dlFileName = report.fileName || `${report.name.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_history.xlsx`;
