...
export const MaintenanceReports: React.FC = () => {
  ...
  const columns = [
    ...
    { key: 'total_work_orders', header: t('total_work_orders') },
    { key: 'completed_count', header: t('completed'), render: (item: MaintenanceReport) => <span className="text-green-600">{item.completed_count}</span> },
    { key: 'overdue_count', header: t('overdue'), render: (item: MaintenanceReport) => <span className={item.overdue_count > 0 ? 'text-red-600 font-bold' : ''}>{item.overdue_count}</span> },
    { key: 'mttr_minutes', header: 'MTTR', render: (item: MaintenanceReport) => `${item.mttr_minutes.toFixed(1)} min` },
    { key: 'total_cost', header: t('total_cost'), render: (item: MaintenanceReport) => `$${item.total_cost.toFixed(2)}` },
  ];
