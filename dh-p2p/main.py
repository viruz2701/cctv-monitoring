#!/usr/bin/env python3
"""
DH-P2P + PTCP Implementation (with --port support)
"""
import argparse
import datetime
import random
...
def main(serial, dtype=0, username=None, password=None, debug=False, rtsp_port=554):
    socketserver = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    socketserver.bind(("0.0.0.0", rtsp_port))
    socketserver.listen(5)
    print(f"Listening on port {rtsp_port}")
    ...
    print(f"Test with: rtsp://127.0.0.1:{rtsp_port}/cam/realmonitor?channel=1&subtype=0")
    while True:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port", type=int, default=554, help="RTSP port to listen on")
    ...
    if args.serial:
        main(args.serial, args.type, args.username, args.password, args.debug, args.port)